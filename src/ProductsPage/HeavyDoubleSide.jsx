...
import HeavyDoubleSideImg from "./productsImg/HEAVY DOUBLE SIDE ABRASIVE BELT GRINDER.jpg";
import { useEffect } from "react";

function HeavyDoubleSide() {
    useEffect(() => {
        window.scrollTo(0, 0);
    }, []);

    const tableData = [
        { model: "KHBG-01", hp: "3", kw: "2.20", phase: "3", rpm: "2800", rubberWheel: "250x50x25", beltSize: "50x2000" },
        { model: "KHBG-02", hp: "5", kw: "3.70", phase: "3", rpm: "2800", rubberWheel: "250x50x25", beltSize: "50x2000" },
    ];
            ...
            <table className="table-auto w-full bg-white border border-gray-300 shadow-md">
                <thead>
                    <tr className="bg-gray-700 text-white">
                        ...
                        <th className="py-2 px-4 border-b text-left">Rubber Wheel Size (mm)</th>
                ...
                <tbody>
                    {data.map((row, index) => (
                        <tr key={index} className={`${index % 2 === 0 ? "bg-gray-100" : "bg-gray-50"} hover:bg-gray-200`}>
                            ...
                            <td className="py-2 px-4 border-b text-gray-800">{row.rubberWheel}</td>
        ...
        <div>
            ...
            <div className="mx-auto px-4 flex flex-col lg:flex-row items-start lg:items-center gap-4 py-6 bg-gray-50">
                <div className="lg:w-[35vw] w-full flex justify-center lg:justify-start">
                    <img
                        src={HeavyDoubleSideImg}
                        className="w-full sm:w-2/3 lg:w-3/4 xl:w-2/3 lg:ml-32 border-2"
                        alt="Heavy Double Side Abrasive Belt Grinder"
                    />
                </div>

                <div className="lg:w-1/2 w-full">
                    <h2 className="text-4xl font-bold mb-6 text-gray-800">
                    HEAVY DOUBLE SIDE ABRASIVE BELT GRINDER
                    </h2>
...
export default HeavyDoubleSide;
